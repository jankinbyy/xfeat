...
private:
    ...
    cv::Mat matchAndDraw(cv::Mat& refFrame, cv::Mat& currentFrame) 
    {
        ...
        cv::Mat mask;
        // USAC_MAGSAC'wn'opencv-4.5.5' not wn opencv-4.2.0'
        //cv::USAC_MAGSAC 是 OpenCV 4.4.0 引入的改进版 RANSAC 方法，用于更鲁棒的参数估计。旧版本不支持
        // cv::Mat H = cv::findHomography(mkpts_0_cv, mkpts_1_cv, cv::USAC_MAGSAC, 10.0, mask, 1000, 0.994);
        cv::Mat H = cv::findHomography(mkpts_0_cv, mkpts_1_cv, cv::USAC_DEFAULT, 10.0, mask, 1000, 0.994);
        //OpenCV 4.2.0 支持 RANSAC、LMEDS、RHO 等，但不支持 USAC_MAGSAC。
        //OpenCV 'v4.4.0+ 支持 USAC_MAGSAC、USAC_FAST 等
